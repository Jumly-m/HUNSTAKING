...
async function loadInitialData(sClass) {
  try {
    ...
    let cObj = new web3Main.eth.Contract(
      _CONTRACT_DATA[_NETWORK_ID].STACKING.abi,
      _CONTRACT_DATA[_NETWORK_ID].STACKING.address
    );
    ...
    $("#total-locked-tokens").html(
      `${totalLockedTokens / 10 ** 18} ${
        _CONTRACT_DATA[_NETWORK_ID].TOKEN.symbol
      }`
    );
    ...
    console.log(error);
    notyf.error(
      `Unable to fetch data from ${_CONTRACT_DATA[_NETWORK_ID].network_name}!\n Please refersh this page.`
    );
  }
}
...
async function connectMe(_provider) {
  try {
    ...
    if (!_comn_res) {
      ...
      $("#referral-link").html(
        `${window?.location?.href}?ref=${selectedAccount}`
      );
...
function copyReferralLink() {
  // Get the dynamic value from the other tag
  var referralValue = document.getElementById("referral-link").innerHTML;

  // Create a temporary input element
  var tempInput = document.createElement("input");
  tempInput.type = "text";
  tempInput.value = referralValue;
  ...
  notyf.success("Referral Link Copied");
}

function toggleNavbar() {
  var div = document.getElementById("collapse-menu");
...
async function getUserData(address) {
  try {
    let oContractStacking = getContractObj();
    let oContractMining = getMiningContractObj();
    ...
    let [
      ...
      oContractToken.methods.balanceOf(address).call(),
      oContractStacking.methods.investors(address).call(),
      oContractStacking.methods.getReward(address).call(),
      oContractMining.methods.miners(address).call(),
      oContractMining.methods.getCurrentReward(address).call(),
      oContractStacking.methods.referralCount(address).call(),
      oContractMining.methods.referralCount(address).call(),
    ]);

    console.log("HUN Balance", balMainUser);

    balMainUser = web3.utils.fromWei(balMainUser, "ether");

    $(".hun-balance").html(balMainUser);

    $("#total-locked-tokens").html(
      web3.utils.fromWei(investorData?.totalStaked, "ether")
    );
    $("#total-locked-tokens").removeClass(".initial-loader");

    $("#user-reward-balance").html(
      `Reward: ${parseFloat(web3.utils.fromWei(totalReward, "ether"))?.toFixed(
        2
      )}`
    );
    $("#user-reward-balance").removeClass(".initial-loader");

    $("#staking-referral-count").html(referralCount);
    $("#mining-referral-count").html(miningReferralCount);

    const totalRewardMining =
      parseFloat(web3.utils.fromWei(minersData?.totalMined)) +
      parseFloat(web3.utils.fromWei(currentReward, "ether"));

    $("#total-mined-tokens").html(totalRewardMining?.toFixed(2));
  } catch (err) {
    console.log(err);
...
async function getGenericData() {
  try {
    let oContractStacking = getContractObj();
    let oContractMining = getMiningContractObj();
    let oContractDonation = getDonationContractObj();

    let [totalInvestors, totalMiners, totalDonation, totalDonatorCount] =
      await Promise.all([
        oContractStacking.methods.totalInvestors().call(),
        oContractMining.methods.totalMiner().call(),
        oContractDonation.methods
          .totalDonation(_CONTRACT_DATA[_NETWORK_ID].TOKEN.address)
          .call(),
        oContractDonation.methods.totalDonator().call(),
      ]);

    $("#num-of-stackers").html(totalInvestors);
    $("#num-of-stackers").removeClass(".initial-loader");

    $("#num-of-miners").html(totalMiners);
    $("#total-donation-amount").html(web3.utils.fromWei(totalDonation));
    $("#total-donator-amount").html(totalDonatorCount);
  } catch (err) {
    console.log(err);
    throw new Error(err?.message);
  }
...
async function donateTokens() {
  try {
    let nTokens = $("#amount-to-donate").val();
    ...
    if (balMainUser < nTokens) {
      ...
      notyf.error(
        `insufficient tokens on ${_CONTRACT_DATA[_NETWORK_ID].network_name}.\nPlease buy some tokens first!`
      );
      return;
    }
    ...
    let balMainAllowance = await oContractToken.methods
      .allowance(selectedAccount, _CONTRACT_DATA[_NETWORK_ID].DONATION.address)
      .call();
    ...
    if (Number(balMainAllowance) < Number(tokenToTransfer)) {
      await approveTokenSpend(
        tokenToTransfer,
        _CONTRACT_DATA[_NETWORK_ID].DONATION.address
      );
    }
    await donateTokenMain(tokenToTransfer, sClass);
  } catch (error) {
    // enableDisableBtns(true);
    showHideBtnsVal("#stake-btn", true);
    console.log(error);
...
async function setStakeMax() {
  try {
    const oContractToken = new web3.eth.Contract(
      _CONTRACT_DATA[_NETWORK_ID].TOKEN.abi,
      _CONTRACT_DATA[_NETWORK_ID].TOKEN.address
    );

    const balanceInWei = await oContractToken.methods
      .balanceOf(selectedAccount)
      .call();

    const balanceInEth = web3.utils.fromWei(balanceInWei, "ether");

    $("#amount-to-stack").val(balanceInEth);
  } catch (err) {
    console.log(err);
    throw new Error(err?.message);
  }
...
async function setDonateMax() {
  try {
    ...
    $("#amount-to-donate").val(balanceInEth);
  } catch (err) {
    console.log(err);
    throw new Error(err?.message);
  }
...
async function claimMiningTokens() {
  try {
    await claimMiningTokenMain();
  } catch (error) {
    // enableDisableBtns(true);
    showHideBtnsVal("#stake-btn", true);
    console.log(error);
...
async function stackTokens() {
  try {
    ...
    let sClass = getSelectedTab();
    let balMainAllowance = await oContractToken.methods
      .allowance(selectedAccount, _CONTRACT_DATA[_NETWORK_ID].STACKING.address)
      .call();
    ...
    await stackTokenMain(tokenToTransfer, sClass);
  } catch (err) {
    // enableDisableBtns(true);
    showHideBtnsVal("#stake-btn", true);
    console.log(error);
    // notyf.dismiss(notification);
    notyf.error(formatEthErrorMsg(err));
  }
}

async function approveTokenSpend(_mint_fee_wei, address) {
  try {
    let gasEstimation;

    try {
      gasEstimation = await oContractToken.methods
        .approve(address, _mint_fee_wei)
        .estimateGas({
    ...
    const trxHash = await oContractToken.methods
      .approve(address, _mint_fee_wei)
      .send({
        from: selectedAccount,
        gas: gasEstimation,
...
async function donateTokenMain(_amount_wei, sClass) {
  let gasEstimation;
  let oContractDonation = getDonationContractObj();

  try {
    gasEstimation = await oContractDonation.methods
      .donate(_CONTRACT_DATA[_NETWORK_ID].TOKEN.address, _amount_wei)
      .estimateGas({
        from: selectedAccount,
      });
    // console.log("gasEstimation", gasEstimation);
    ...
    console.log(error);
    notyf.error(formatEthErrorMsg(error));
    showHideBtnsVal("#donate-btn", true);
    return;
  }
...
async function stackTokenMain(_amount_wei, sClass) {
  ...
  referralAddress =
    getParameterByName("ref") || "0x0000000000000000000000000000000000000000";
  ...
  try {
    gasEstimation = await oContractStacking.methods
      .investAmount(referralAddress, _amount_wei)
      .estimateGas({
        from: selectedAccount,
      });
    // console.log("gasEstimation", gasEstimation);
...
async function mineTokenMain() {
  ...
  const minerData = await oContractMining.methods
    .miners(selectedAccount)
    .call();

  let value = web3.utils.toWei("0", "ether");
  if (minerData?.miner == nullAddress) {
    const fee = await oContractMining.methods.MINING_FEE().call();
    value = fee;
  }
  try {
    gasEstimation = await oContractMining.methods
      .mineToken(referralAddress)
      .estimateGas({
        from: selectedAccount,
        value: value,
      });
...
async function claimMiningTokenMain() {
  let gasEstimation;

  let oContractMining = getMiningContractObj();

  const reward = await oContractMining.methods
    .getWithdrawableCurrentReward(selectedAccount)
    .call();

  try {
    gasEstimation = await oContractMining.methods.withdrawReward().estimateGas({
      from: selectedAccount,
    });
    // console.log("gasEstimation", gasEstimation);
  } catch (error) {
    console.log(error);
    if (error?.toString()?.includes("Minimum withdraw")) {
      notyf.error("Minimum withdraw is 20,000 HUN");
    } else {
      notyf.error(formatEthErrorMsg(error));
    }
    showHideBtnsVal("#stake-btn", true);
...
async function unstackTokens() {
  try {
    ...
    if (balMainUser < nTokens) {
      ...
      notyf.error(
        `insufficient staked tokens on ${_CONTRACT_DATA[_NETWORK_ID].network_name}!`
      );
      return;
    }
...
async function claimTokens() {
  try {
    ...
    let rewardBal = await oContractStacking.methods
      .getWithdrawableReward(selectedAccount)
...
async function refreshBal(sClass) {
  ...
  $("#user-main-balance")
    .attr("bal", nBalMainW)
    .html(`Balance: ${nBalMainW} ${_CONTRACT_DATA[_NETWORK_ID].TOKEN.symbol}`);

  let oContractStacking = getContractObj(sClass);

  let nBalStack = await oContractStacking.methods
    .getUser(selectedAccount)
    .call();
  ...
  $("#user-staked-balance")
    .attr("bal", nBalStackW)
    .html(`${nBalStackW} ${_CONTRACT_DATA[_NETWORK_ID].TOKEN.symbol}`);
...
async function getTxhistory(sClass) {
  try {
    ...
    let stakingContractAddress = _CONTRACT_DATA[_NETWORK_ID].STACKING.address;
      ...
      for (let i = 0; i < res.length; i++) {
        ...
        $("#tx-history-body tbody").append(`
          ...
          _CONTRACT_DATA[_NETWORK_ID].TOKEN.symbol
                        ...
                        <td>${res[i].status}</td>
                        <td style="justify-content: center !important;"><a href="${
                          _CONTRACT_DATA[_NETWORK_ID].explorer_url
                        }/${
          res[i].transaction_hash
        }"><i class="fa fa-external-link h6" aria-hidden="true"></i></a></td>
                    </tr>
                `);
      }
    } else {
