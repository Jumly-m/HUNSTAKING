...
async function commonInjectedConnect(_provider1, _provider_name) {
...
function getDonationContractObj() {
  return new web3.eth.Contract(
    _CONTRACT_DATA[_NETWORK_ID].DONATION.abi,
    _CONTRACT_DATA[_NETWORK_ID].DONATION.address
  );
}

function getMiningContractObj() {
  return new web3.eth.Contract(
    _CONTRACT_DATA[_NETWORK_ID].MINING.abi,
    _CONTRACT_DATA[_NETWORK_ID].MINING.address
  );
}
